...
class scalable_linear_v2(nn.Module):
    def __init__(self, dim_in, dim_out, bias=True):
        super().__init__()
        self.linear = nn.Linear(dim_in, dim_out, bias)
        #self.scaling = nn.Parameter(torch.ones(dim_out))
    ...
    def forward(self, x):
        normlist = torch.linalg.norm(self.linear.weight, dim=-1)
        return self.linear(x)* 1/normlist
    ...
    def normalize(self, old_norms=None):
        ...
        with torch.no_grad():
            ...
            #normlist = torch.sum(torch.abs(self.linear.weight.data)**p, dim=-1).unsqueeze(-1)**(1/p)
            self.linear.weight.data.mul_(1/normlist)
            self.linear.bias.data.mul_(normlist.squeeze())
        return normlist


class MLP_normed_v2(nn.Module):
    def __init__(self, dims=[784, 256, 64, 64, 62], bias=True):
        super().__init__()
        self.layers = nn.ModuleList([scalable_linear_v2(dims[i], dims[i+1], bias=bias) for i in range(len(dims)-1)])
        self.outnorms = nn.Parameter(torch.ones(dims[-1]), requires_grad=False)
    ...
    def normalize(self):
        ...
        for i, layer in enumerate(self.layers):
            ...
            returnnorms[i,1] = torch.min(old_norms)
        with torch.no_grad():
            self.outnorms.data.mul_(1/self.outnorms.data)
            self.outnorms.data.mul_(old_norms.squeeze())
        return returnnorms
